...
def main(_):
    ...
    if not all([os.listdir(generations_base), os.listdir(losses_base)]):
        for trial in range(_NUM_TRIALS.value):
            ...
            print(generations)
            likelihoods = [ np.sum([np.where(generation[0][-50:] == generation[j+1][-50:], 1, 0).sum()/weights[j] for j in range(3)]) for generation in generations_groups ]
            # for gen in generations_groups:
            #     unique_token = np.unique(gen[0][-50:])
            #     likelihood = [1 - 1/len(unique_token)*np.sum([ abs((gen[0][-50:] == token).sum() - (gen[i][-50:] == token).sum())/(gen[0][-50:] == token).sum() for token in unique_token]) for i in range(1, 4)]
