...
def generate_for_prompts(
    ...
    for i, off in tqdm(enumerate(range(0, len(prompts), batch_size))):
        ...
        subset_prompt_batch = np.stack(subset_prompt_batch, axis=1).reshape(size*4, _SUFFIX_LEN)
    ...
    return np.atleast_2d(generations), 0 #np.atleast_2d(losses).reshape((len(generations), -1))
...
def main(_):
    ...
    for generations_per_prompt in [1, 5]:
        ...
        with open("guess%d_subset.csv"%generations_per_prompt, "w") as file_handle:
