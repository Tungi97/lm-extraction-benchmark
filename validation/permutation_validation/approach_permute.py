...
def main(_):
    ...
    if not all([os.listdir(generations_base), os.listdir(losses_base)]):
        for trial in range(_NUM_TRIALS.value):
            ...
            for gen in generations_groups:
                ...
                likelihood = [1/len(unique_token)*np.sum([ abs((gen[0][-50:] == token).sum() - (gen[i][-50:] == token).sum())/(gen[0][-50:] == token).sum() for token in unique_token]) for i in range(3)]
    ...
    for generations_per_prompt in [1, 5]: 
        ...
        with open("guess%d_permute10.csv"%generations_per_prompt, "w") as file_handle:
